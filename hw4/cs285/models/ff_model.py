...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = (obs_unnormalized - obs_mean) / (1e-8 + obs_std)
        acs_normalized = (acs_unnormalized - acs_mean) / (1e-8 + acs_std)
        ...
        delta_pred = (delta_std * delta_pred_normalized) + delta_mean
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        delta = next_obs - obs
        target = (
            (delta - ptu.from_numpy(data_statistics["delta_mean"]))
        ...
        loss = self.loss(delta_hat, target)
